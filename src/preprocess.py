...
def preprocess_data(data, method="standard"):
    ...
    """
    print(f"🛠️ Preprocessing data with {method} method...")
    ...
    if method == "standard":
        ...
        try:
            X["length_of_stay"] = (pd.to_datetime(X["checkout_month"] + " " + X["checkout_day"].astype(str)) - 
                                 pd.to_datetime(X["arrival_month"] + " " + X["arrival_day"].astype(str))).dt.days
            numerical_cols.append("length_of_stay")
        except Exception as e:
            print(f"⚠️ Warning: Could not compute 'length_of_stay' feature: {e}")
    else:
