...
def clean_price_column(df, step_1=True, step_2=True):
    """
    ...
    """
    column_name = 'price'  # Define the column name to be cleaned
    exchange_rate = 1.35  # Exchange rate for USD to SGD
    threshold = 1500  # Threshold for outlier detection
    ...
    try:
        ...
        if step_1 == True:
            # Carry out basic cleaning on the 'price' column
            df_cleaned = basic_cleaning(df_cleaned, column_name, exchange_rate)

        if step_2 == True:
            # Remove outliers from the 'price' column
            df_cleaned = remove_outliers(df_cleaned, column_name, threshold)
...
def basic_cleaning(df, column_name, exchange_rate):
    """
    Perform basic cleaning on the 'price' column by:
    1. Dropping rows with missing values.
    2. Removing currency prefixes (e.g., "SGD$" or "USD$").
    3. Converting the cleaned price column to numeric.

    Parameters:
        df (pd.DataFrame): The input DataFrame.
        column_name (str): The name of the column to be cleaned.
        exchange_rate (float): The exchange rate for converting USD to SGD.
...
def remove_outliers(df, column_name, threshold=1500):
    """
    Remove outliers from the DataFrame based on a specified threshold.
    ...
    # Step 1: Drop outlier rows
    print(f"      └── Removing outliers the {column_name} column...")
    is_president_suite = df_cleaned['room'] == 'President Suite'
    is_low_price = df_cleaned['price'] < threshold
    is_outlier = is_president_suite & is_low_price
    is_not_outlier = ~is_outlier
    df_cleaned = df_cleaned[is_not_outlier]
